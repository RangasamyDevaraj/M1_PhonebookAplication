...
void test_add_contact(void) {
  contact_t new_contact = {"Add_user", "12345", "user1@email.com","address1"};
  TEST_ASSERT_EQUAL(0, add_contact(&new_contact));
}

void test_delete_contact(void) {
  contact_t new_contact = {"Delete_user", "12345", "user1@email.com","address1"};
  TEST_ASSERT_EQUAL(0, add_contact(&new_contact));
  TEST_ASSERT_EQUAL(0, delete_contact(new_contact.name));
}

void test_modify_contact(void) {
  contact_t new_contact = {"Modify_User", "12345", "user1@email.com","address2"};
  contact_t new_contact1 = {"Modified_User", "345", "new@email.com"."address3"};
  TEST_ASSERT_EQUAL(0, add_contact(&new_contact));
...
void test_search_contact(void) {
  contact_t new_contact = {"Search_User", "345", "new@email.com","addresss"};
  TEST_ASSERT_EQUAL(0, add_contact(&new_contact));
